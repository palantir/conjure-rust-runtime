    ...
    fn build_raw_client(&self, builder: &Builder<Self>) -> Result<Self::RawClient, Error> {
        ...
        let client_config = match (
            ...
            (Some(cert_file), Some(key_file)) => {
                ...
                let key_file = File::open(key_file).map_err(Error::internal_safe)?;
                let mut key_file = BufReader::new(key_file);
                let mut items =
                    rustls_pemfile::read_all(&mut key_file).map_err(Error::internal_safe)?;
                ...
                let key_der = match items.pop().unwrap() {
                    Item::RSAKey(buf) | Item::PKCS8Key(buf) => PrivateKey(buf),
                    _ => {
                        return Err(Error::internal_safe(
                            "expected a PKCS#1 or PKCS#8 private key",
                        ))
                    }
                };
